...
function formatOutput(text) {
  ...
  if (ideaMatches.length === 0) ideaMatches = [text];
  ideaMatches = ideaMatches.slice(0, 3);

  return ideaMatches.map((idea, idx) => {
    let lines = idea.split("\n").map(l => l.trim()).filter(Boolean);

    // Proper name extraction from "Project Idea X: Name"
    let nameLine = lines.find(l => /Project Idea \d+[:\-]/i.test(l)) || lines[0];
    let nameMatch = nameLine.match(/Project Idea \d+[:\-]?\s*(.*)/i);
    let name = nameMatch ? nameMatch[1].trim() : "Unnamed Project";
    ...
    let contentHtml = sections.map(sec => {
      ...
      if (lines.some(l => /^[-•]/.test(l))) content = "<ul>" + lines.map(l => `<li>${l.replace(/^[-•]\s*/, "")}</li>`).join("") + "</ul>";
      else content = lines.map(l => `<p>${l}</p>`).join("");
