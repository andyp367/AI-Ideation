...
Generate 3 computer engineering project ideas. For each, provide:
- Name
- General Description
...
function formatOutput(text) {
    ...
    // Split by "Project Idea" or fallback
    let ideaMatches = text.split(/Project\s*Idea\s*\d+[:\-]?\s*/i).filter(s => s.trim());
    // Ensure 3 projects are always present
    while (ideaMatches.length < 3) ideaMatches.push("Unnamed Project\nGeneral Description: N/A\nRequired Technologies & Budget Breakdown: N/A\nTimeframe Breakdown: N/A\nComplexity & Skills Needed: N/A\nSimilar Products: N/A\nNovel Elements: N/A");
    ...
    return ideaMatches.map(idea => {
        ...
        let name = nameMatch ? nameMatch[1].trim() : nameLine.trim() || "Unnamed Project";
